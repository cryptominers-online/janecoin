JaneCoin-qt: Qt4 GUI for JaneCoin
===============================
...
Alternatively, install Qt Creator and open the `JaneCoin-qt.pro` file.

An executable named `JaneCoin-qt` will be built.
...
A warning for people using the *static binary* version of JaneCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of JaneCoin is linked against libdb5.3.
...
installed causes JaneCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
